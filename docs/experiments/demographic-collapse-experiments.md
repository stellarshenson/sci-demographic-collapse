...
| E41 | calibration extension to multi-observable targets (research round: Wave 1 targets complete, Wave 2 blocker resolution in flight, Wave 3 implementation user-gated; `docs/e41-acceptance-criteria.md`) | - | 0 | research round (in progress) |
| **total** | **40 rounds (E1-E40)** | **H1-H392** | **392** | **211 / 109 / 69** (+2 reframed, 1 inconclusive; E13 audit 12/13; E40 audit 4 confirmed / 3 documented) |
...
## E41 - Calibration extension to multi-observable targets (research round, in progress)
...
**Wave 2 - blocker resolution (in flight)**: the 18 findings split into 11 research gaps (data hunts -
several fillable from the on-disk WPP CSVs, the rest web-sourced per the standing E41 authorization),
6 protocol/definitional amendments (the critics supplied the fixes verbatim), and 1 guard-suite
reconciliation. Each finding must land as RESOLVED, AMENDED, or ACCEPTED-RESIDUAL in a disposition
table (`reports/e41_blocker_resolutions.json`), the dossier is then re-issued as v2, and the same two
adversarial critics re-judge it - the bar is zero BLOCKERs and no MAJOR outside an explicitly accepted
residual, looped per the rounds protocol. Wave-2 results will be appended here when the re-critique
lands; nothing in this wave touches the model.

**Wave 3 - implementation (not started, user-gated)**: anchor replacement in dependency order, the
observability harness, the guard-suite re-baselining, and the rejection backtest - only after Wave 2
