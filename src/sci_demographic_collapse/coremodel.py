...
def load_unwpp(data_dir="data/raw/unwpp", y0=1990, y1=2023, regions=None, varid=2):
    ...
    def _load(name):
        return pl.read_csv(data_dir / f"{name}.csv", infer_schema_length=5000).filter(
            pl.col("VarID") == varid
        )

    pop, life, fert, ind = (
        _load("population_by_age1"),
        _load("life_table_both"),
        _load("fertility_by_age1"),
        _load("demographic_indicators"),
    )
    out = {}
...
def leslie_step(nf, nm, Sx, asfr, srb, mig=None):
    ...
    S0 = Sx[0]
    nf_s = torch.zeros_like(nf)
    nm_s = torch.zeros_like(nm)
...
def project(region, mig_schedule=None, asfr_override=None, sx_override=None, dev=DEV):
    ...
    for k in range(n):
        ...
        pyr_f[k] = nf.cpu().numpy()
        pyr_m[k] = nm.cpu().numpy()
        if k == n - 1:
            break
