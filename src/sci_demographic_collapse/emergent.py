...
# behavioural rate constants (calibrated in E19)
PARAMS = dict(
...
class EmergentModel:
    ...
    def _estep(self, nm, st, force, tn, dep_pen, dt=0.25):
        p = self.P
        C, rv, Pb, tau, S = st
        fS, fC, fPb, fTau, fRV = force
    ...
    def run(self, region: str, force=None, years: int = 102) -> dict:
        ...
        """
        if force is None:
            force = lambda yr: [0.0] * 5  # noqa: E731
        st = np.array([C0[region], RV0[region], self.PB0[region], REAL[region][1], S0[region]])
        R = self.REG[region]
        ...
        for yr in range(years):
            ...
            dtau = 0.0
            for _ in range(4):
                st, dt_ = self._estep(region, st, f, yr / 100.0, dep_pen)
                dtau += dt_
            C, rv, Pb, tau, S = st
            ...
            Ctr.append(C)
            Ttr.append(tfr)
        return dict(C=np.array(Ctr), TFR=np.array(Ttr), Cend=float(C), tfr=float(tfr),
    ...
    @staticmethod
    def forcing(fS=0.0, fC=0.0, fPb=0.0, fTau=0.0, fRV=0.0, mag=1.0, start=0, durable=True):
        """Build a forcing function from explicit per-channel coefficients."""
        def fy(yr):
