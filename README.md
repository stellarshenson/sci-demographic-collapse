...
<!-- The one lever that bends: the marriageable-men lever pulled in three countries (Korea/Germany/USA). Figure shows the illustrative broad-based bend; E36's saturating re-simulation qualifies it - the lever BENDS but does not RECOVER any region alone (drive ceiling sits below the hold threshold), and a concentrated men-only lift roughly halves the effect via the inequality + hypergamy counter-terms, so the strong form is broad-based and gender-balanced. Parametrised in notebooks/17 + the visuals code. -->
...
That value ranking hides the male-prospects lever precisely because it is the costliest - yet it is the strongest single *positive* lever in the model. Here is its predicted strength across nations: a broad-based restoration of young men's economic prospects lifts the birth rate by **+0.27** children in Korea and as much as **+0.43** in Germany. It bends every country, and - tellingly - most where there is still room to move, least in the deepest basin (E36).

<!-- One lever, every nation: the basic broad-based male-prospects lever (drive-alone, single lever, NOT the fanout), ΔTFR at 2125 across the six calibrated nations - Korea +0.27, France +0.29, Italy +0.38, Japan +0.41, USA +0.41, Germany +0.43 (E36-H357; reproduced in nb17 from EmergentModel/run_cal, matches the E36 triad exactly). Companion to the cheapest-fixes chart: the lever missing from the value ranking is the strongest positive lever and bends every nation, most near the ridge. Parametrised in notebooks/17 + the visuals code. -->
![The basic male-prospects lever's predicted lift in the birth rate across six nations - it bends every one, from +0.27 in Korea to +0.43 in Germany, most where there is still room to move](reports/figures/story_male_prospects.png)
...
10. **Hypothesised and pre-registered interventions** - we wrote down 348 testable claims across thirty-five rounds (labelled E1-E35), most of them candidate fixes, and committed to what would count as success *before* running each test, so a result can't be quietly reinterpreted as a win after the fact; each was fanned out across the board with its interactions, reinforcements and cancellations measured
...
- **28 notebooks** (`notebooks/01…28`) - the whole investigation, step by step
- **The evidence log** - all 348 questions put to the model and how each turned out: [`docs/experiments/demographic-collapse-experiments.md`](docs/experiments/demographic-collapse-experiments.md)
...
├── notebooks           <- 01…28, the investigation end to end
